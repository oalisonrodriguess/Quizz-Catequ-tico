...
import { Moon, Sun, CheckCircle2, Trash2, BookmarkCheck } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (selectedConfig.topic === 'SAVED') {
        ...
        if (questions.length === 0) {
          alert("Você não tem perguntas salvas!");
          setIsLoading(false);
          return;
        }
      } else {
      ...
      alert("Houve um erro ao conectar com a Catequese Digital. Tente novamente.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAnswer = (index: number, isCorrect: boolean) => {
    setQuizState(prev => {
      ...
      // Just ensure it's recorded if it wasn't (though UI prevents clicking)
    ...
    <div className={isDarkMode ? "dark" : ""}>
      <div className="min-h-screen transition-colors duration-300 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="z-10 w-full flex justify-center">
          ...
          {gameState === 'playing' && quizState.questions.length > 0 && (
            <QuizScreen
